...
Children made relatively few errors (M = `r round(ErrorData_E1_Summary$meanError,2)`%) suggesting they understood the task instructions, though 3-year-olds made more errors than 4-year-olds (main effect of age: 3-year-olds: M = `r round(ErrorData_E1_SummarybyAge$meanError,2)[1]`%, 4-year-olds: M = `r round(ErrorData_E1_SummarybyAge$meanError,2)[2]`%, F(1,75) =  `r round(aov.errors$ANOVA$F,2)[1]`,  p = `r round(aov.errors$ANOVA$p,2)[1]`, ηG2 = `r round(aov.errors$ANOVA$ges,2)[1]`). In addition, children showed evidence for the Size-Stroop effect in their errors; they made more errors on incongruent than congruent displays (main effect of trial type: 
...
F(1,77) = `r round(aov.errors$ANOVA$F,2)[2]`, p = `r round(aov.errors$ANOVA$p,2)[2]`, ηG2 = `r round(aov.errors$ANOVA$ges,2)[2]`). The Size-Stroop effect was apparent throughout this age range; there was no interaction between age group and trial type (F(1,75) = `r round(aov.errors$ANOVA$F,2)[3]`, p = `r round(aov.errors$ANOVA$p,2)[3]`, ηG2 < `r round(aov.errors$ANOVA$ges,2)[3]`).  
