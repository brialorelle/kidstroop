...
errorSubs <- read.csv(e1_fileName) %>%
  ...
  filter(trial>10)  %>%
  group_by(Subject) %>%
  dplyr::summarize(countTrials = length(RT)) %>%
  dplyr::summarize(avgTrials = mean(countTrials), minTrials=min(countTrials), maxTrials=max(countTrials))
...
# now report out summary for the RT demographics text
trialSummary <- includedSubs_RT %>%
  group_by(Subject) %>%
...
ggsave("Figure3-E1andE2-pdf", width = 10, height = 6,unit =  "in", plot = compiledPlot, path="./figures/", device = "pdf",dpi = 300)
```
